#include "two-vertex-connected-square.hpp"
#include "util.hpp"
#include "graph.hpp"
#include <iostream>
...
void TwoVertexConnectedSquare::output(std::bitset<N>& C) {
    num_count_++;
    // cout << "{";
    // integer i = 0;
...
optional<bitset<N>> TwoVertexConnectedSquare::computeMrs(bitset<N> &C, bitset<N> &X) {
    // enumerate Can(C)
    ...
    for (auto& candidate: _candidates_) {
        ...
        G_C_minus_Y.enumerate_articulation_points();
        if (G_C_minus_Y.articulation_points_.empty() && G_C_minus_Y.is_connected()) {
            return Y_bit;
        }
    }
    // return
...
void TwoVertexConnectedSquare::_enumerate_candidate(bitset<N>& C) {
    _candidates_.clear();
    // generate G[C]
    ...
    for (integer u = 0; u < G_C.n_; ++u) {
        if (G_C.V_[u].deg_ > 2) {
            // {u} is candidate
            auto u_in_G = G_C.vmap_inv_.find(u)->second;
            auto Y_vec = { u_in_G };
            auto Y_bit = bitset<N>();
            Y_bit.set(u_in_G);
    ...
    for (auto &connected_component : G_C_minus_CAN_G_2.connected_components_) {
        ...
        for (auto &v: connected_component) {
            auto v_in_G = G_C_minus_CAN_G_2.vmap_inv_.find(v)->second;
...
std::vector<std::bitset<N>> TwoVertexConnectedSquare::computeAllMrs(std::bitset<N> &C, std::bitset<N> &X) {
    // enumerate Can(C)
    _enumerate_candidate(C);
    // decide wheter Y \in Can(C) is MRS
