#include "two-edge-connected.hpp"
#include "util.hpp"
#include "graph.hpp"
#include <iostream>
#include <algorithm>
...
void TwoEdgeConnected::output(std::bitset<N>& C) {
    num_count_++;
    // cout << "{";
    // integer i = 0;
...
optional<bitset<N>> TwoEdgeConnected::computeMrs(bitset<N> &C, bitset<N> &X) {
    #ifdef __DEBUG_
        cout << "TRACE: TwoEdgeConnected::computeMrs" << endl;
    #endif
    // generate G[C]
...
optional<bitset<N>> TwoEdgeConnected::_computeMrsOfVComponent(Graph& G_C_v, bitset<N>& C_v, bitset<N>& X, set<integer>& articulation_points, Graph& G_C, bitset<N>& articulation_points_bit) {
    #ifdef __DEBUG_
        cout << "TRACE: TwoEdgeConnected::_computeMrsOfVComponent" << endl;
    #endif
...
void TwoEdgeConnected::_computeAllMrsOfVComponent(graph::Graph &G_C_v, std::bitset<N> &C_v, std::bitset<N> &X, std::set<integer> &articulation_points, graph::Graph& G_C, std::bitset<N>& articulation_points_bit, std::vector<std::bitset<N>>& MRSs) {
    auto C_v_in_G = G_C.inverse_bitset(C_v);
...
void TwoEdgeConnected::_find_vertices_of_auxiliary_graph(std::bitset<N>& C_v_in_G) {
    #ifdef __DEBUG_
        cout << "TRACE: TwoEdgeConnected::_find_vertices_of_auxiliary_graph" << endl;
    #endif
...
ogdf::Graph TwoEdgeConnected::_generate_auxiliary_graph_and_set_candidates(std::bitset<N>& C_v_in_G, Graph& G_C_v, Graph& G_C) {
    #ifdef __DEBUG_
        cout << "TRACE: TwoEdgeConnected::_generateAuxiliaryGraphAndSetCandidates" << endl;
    #endif
...
void TwoEdgeConnected::_decide_whether_candidates_is_mrs(ogdf::StaticSPQRTree &SPQRTree, std::vector<bool> &is_mrs) {
    #ifdef __DEBUG_
        cout << "TRACE: TwoEdgeConnected::_decide_whether_candidates_is_mrs" << endl;
    #endif
    ...
    //     cout << i << ": ";
    //     OutputBitsetForDebug(std::get<1>(_candidates_gt_two_[i]));
    // }
    // for (integer i = 0; i < b; ++i) {
    //     cout << a + i << ": ";
    //     OutputBitsetForDebug(std::get<1>(_candidates_eq_two_[i]));
    // }
    // for (integer i = 0; i < c; ++i) {
    //     auto [l, r] = _edges_incident_to_can_gt_two_[i];
    //     cout << a + b + i << ": (" << l << "," << r << ")" << endl;
    // }
    ...
    for (auto &v : SPQRTree.nodesOfType(ogdf::SPQRTree::NodeType::SNode)) {
        ...
        for (auto& e: g.edges) {
            if (SPQRTree.skeleton(v).isVirtual(e)) {
                auto [x, y] = e->nodes();
                auto [l, r] = minmax(SPQRTree.skeleton(v).original(x)->index(), SPQRTree.skeleton(v).original(y)->index());
                if (l < a + b && a <= r) {
                    // l is CAN,
                    // r is auxiliary vertex, and
    ...
    for (auto &v : SPQRTree.nodesOfType(ogdf::SPQRTree::NodeType::SNode)) {
        auto g = SPQRTree.skeleton(v).getGraph();
        auto number_of_auxiliary_vertex = 0;
        auto number_of_adjacent_auxiliary_vertex = vector<integer>(g.numberOfNodes(), 0);
        ...
        for (auto &e : g.edges) {
            ...
            if (x_original >= a) {
                // x is auxiliary vertex
                ++number_of_auxiliary_vertex;
                ++number_of_adjacent_auxiliary_vertex[y->index()];
            }
            if (y_original >= a) {
                // y is auxiliary vertex
                ++number_of_auxiliary_vertex;
                ++number_of_adjacent_auxiliary_vertex[x->index()];
            }
        }
        number_of_auxiliary_vertex /= 2;
        for (integer i = 0; i < g.numberOfNodes(); ++i) {
            if (number_of_adjacent_auxiliary_vertex[i] != number_of_auxiliary_vertex) {
                if (original_id[i] >= a + b) {
                    continue; // not candidate
                }
                if (original_id[i] >= a && original_id[i] < b + a) { // if i is two-deg path
                    if (number_of_adjacent_auxiliary_vertex[i] + 1 == number_of_auxiliary_vertex) {
...
tuple<integer, integer> TwoEdgeConnected::_compute_boundaries(graph::Graph& G_C_v, integer v) {
    #ifdef __DEBUG_
        cout << "TRACE: TwoEdgeConnected::_compute_boundaries" << endl;
        cout << "\tinput v: " << v << endl;
...
optional<bitset<N>> TwoEdgeConnected::_computeMrsOfCycle(Graph& G_C_v, bitset<N>& C_v, bitset<N>& X, set<integer>& articulation_points, Graph& G_C) {
    #ifdef __DEBUG_
        cout << "TRACE: TwoEdgeConnected::_computeMrsOfCycle" << endl;
    #endif
...
void TwoEdgeConnected::_enumerate_candidate(bitset<N>& C) {
    #ifdef __DEBUG_
        cout << "TRACE: TwoEdgeConnected::_enumerate_candidate" << endl;
    #endif
...
void TwoEdgeConnected::_enumerate_edges_itcgt_after_candidates(std::bitset<N> &C) {
    #ifdef __DEBUG_
        cout << "TRACE: TwoEdgeConnected::_enumerate_edges_itcgt_after_candidates" << endl;
    #endif
...
std::vector<std::bitset<N>> TwoEdgeConnected::computeAllMrs(std::bitset<N> &C, std::bitset<N> &X) {
    #ifdef __DEBUG_
        cout << "TRACE: TwoEdgeConnected::computAllMrs" << endl;
    #endif
